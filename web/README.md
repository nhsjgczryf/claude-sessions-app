...
也是一个 **PWA**：在手机 Chrome / Safari 里访问后，可以"添加到主屏"作为独立应用使用（全屏、没浏览器地址栏、应用切换器里独立卡片）；自带触屏键盘工具栏（Tab / Esc / 方向键 / Ctrl 组合 / 常用 shell 符号）和系统**相册**图片粘贴。
...
- **Android Chrome**：访问站点 → 右下角会弹"Install app"小芯片，点一下；或菜单 → 安装应用。装上以后桌面有图标，打开是独立全屏窗口，没浏览器地址栏。
- **iOS Safari**：分享菜单（中间方块带↑） → "添加到主屏幕"。iOS 不支持自动 install 提示，必须从分享菜单走。装好后行为同 Android。
- 装完之后，多设备登录、推送 cookie、心跳、tmux 持久化都跟在 desktop 浏览器里一样工作。

### 键盘工具栏

打开终端 tab 时，触屏设备底部会出现一条横向滚动的工具栏：
...
- **Ctrl** 是 one-shot：点一下会变高亮（armed），**下一个**软键盘字母会被转成 Ctrl+字母（比如 armed + 软键盘"r" → `^R`），用完自动清除。
- 工具栏上的 `^C` / `^D` / `^Z` / `^L` / `^R` 是预设的常用 Ctrl 组合，单击直接发送，省得每次 arm + type。
- 方向键 / Home / End / Tab / Esc 直接发对应的 ANSI 转义序列。
- 字符按钮（`|` `~` `>` 等）是软键盘上不方便切换面板才能输入的字符，发当前光标位置。
- 桌面浏览器自动不显示。如果你在桌面浏览器想强制看到工具栏：URL 加 `?keybar=force`；想关掉：`?keybar=off`。

### 相册图片粘贴

在触屏设备上点侧边栏的 **Img** 按钮 → 弹出系统相册选择器（Android 上是图库 + 文件管理器、iOS 上是照片库 + 文件 app）。选完会上传到服务器，路径插入到当前 Claude 会话——本地 session 直接粘路径，SSH session 先 SCP 到远端再粘远端路径。

支持的格式：PNG / JPEG / WebP / GIF / HEIC（iOS） / BMP，单张限制 25MB。

桌面浏览器上 Img 按钮还是先尝试剪贴板，剪贴板里没图才落到文件选择器。

### 离线 shell 缓存

Service Worker 缓存了页面骨架（HTML / CSS / JS / xterm 库 / 图标）。完全断网时 PWA 仍然能打开 UI，看到"WebSocket disconnected"提示；网络恢复后自动重连。终端流量本身是在线的，没法离线工作——这只是让"应用打不开"的尴尬不会出现。
