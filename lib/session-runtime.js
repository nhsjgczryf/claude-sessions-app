...
//
// `flavor` picks the shell dialect of the fallback: 'sh' (default) for every
// remote/ssh command and non-Windows local shells, 'powershell' for Windows
// local sessions — the command gets typed into powershell.exe, and Windows
// PowerShell 5.1 has neither `||` nor `{ a; b; }` compound commands, so the
// sh form is a hard parse error there ("标记"||"不是此版本中的有效语句分隔符").
function withClaudeResume(claudeCmd, sessionId, resume, flavor) {
  ...
  if (flavor === 'powershell') {
    return `${claudeCmd} --resume ${sessionId}; if ($LASTEXITCODE -ne 0) { ${claudeCmd} --session-id ${sessionId} }`;
  }
...
function buildLocalCommand(session) {
  ...
  if (hasClaude) {
    const claudeArgs = session.claude_args ? ` ${session.claude_args}` : '';
    const raw = `${session.claude_cmd.trim()}${claudeArgs}`;
    // Local commands are typed into the PTY's shell — powershell.exe on
    // Windows — so the resume fallback must match that dialect.
    const flavor = process.platform === 'win32' ? 'powershell' : 'sh';
    parts.push(
      useTmux ? raw : withClaudeResume(raw, session._claude_session_id, session._resume, flavor)
    );
  }
