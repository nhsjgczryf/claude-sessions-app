...
function buildSshCommand(session) {
  ...
  if (persistent) {
    ...
    // process inherits a bare $PATH / no PYENV_ROOT / no proxy vars. Wrap
    // everything in `bash -lc` so login init runs before we cd + exec claude.
    const inner = setupParts.join(' && ');
    remoteCmd = `exec bash -lc ${shellQuote(inner)}`;
  } else {
