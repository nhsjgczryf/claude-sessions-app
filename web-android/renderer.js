  ...
  el.addEventListener('click', (e) => {
    ...
    switch (b.dataset.sel) {
      ...
      case 'preview': {
        ...
        if (!sel) { notify('先选中一个文件路径', 'info'); return; }
        const id = tab.id;
        exitSelectMode();
...
// ============================================================================
//
// Tier 0: read a file over whichever transport the active tab uses and
// render it — markdown (+LaTeX), images, or plain text. No editing; the
// terminal's claude does edits far better than any embedded editor.

// Per-transport file read. Resolves { base64, size, truncated }.
const FileBridge = {
  async read(tab, filePath, maxBytes) {
    const max = maxBytes || (1024 * 1024);
    if (tab.kind === 'local')  return LocalShellBridge.readFile(filePath, max);
    if (tab.kind === 'remote') return WebSocketBridge.fsRead(tab.id, filePath, max);
    return SshBridge.sftpGet(tab.id, filePath, max);
  },
};
